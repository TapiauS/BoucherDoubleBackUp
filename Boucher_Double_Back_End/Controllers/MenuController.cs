...
namespace Boucher_Double_Back_End.Controllers
{
    ...
    [ApiController]
    public class MenuController : APIController<Menu>
    {
        public MenuController(ILogger _logger) : base(_logger)
        {
        }
            ...
            try
            {
                if (ConnectedUser.ContainsKey(HttpContext.Request.Headers["X-CSRF-Token"]))
                {
                    ...
                    if (file != null)
                    {
                        ...
                        var imagePath = "images/menu/" + uniqueFileName;
                        Menu menu = await DAO.GetByIdAsync(id);
                        if (System.IO.File.Exists("images/menu/" + menu.PicturePath))
                            System.IO.File.Delete("images/menu/" + menu.PicturePath);
                        menu.PicturePath = uniqueFileName;
                        if (await DAO.UpdateAsync(menu))
                        {
        ...
        public IActionResult GetImage(string pictureName)
        {
            ...
            {
                logger.LogError(ex, "Error while retriewing an image");
                return BadRequest();
            }
        }
    }
